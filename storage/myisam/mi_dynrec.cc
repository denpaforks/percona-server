  ...
  for (rec = info->s->rec, end_field = rec + info->s->base.fields;
    ...
    if ((type = (enum en_fieldtype)rec->type) != FIELD_NORMAL &&
      ...
      if (type == FIELD_VARCHAR) {
        ...
        if (from + length > from_end) goto err;
      ...
      if (flag & bit) {
        if (type == FIELD_BLOB || type == FIELD_SKIP_ZERO)
          ...
          if (rec->length > 255 && *from & 128) {
            if (from + 1 >= from_end) goto err;
            length = (*from & 127) + ((uint)(uchar) * (from + 1) << 7);
            from += 2;
          } else {
            if (from == from_end) goto err;
            length = (uchar)*from++;
          }
          min_pack_length--;
...
ulong _my_calc_total_blob_length(MI_INFO *info, const uchar *record) {
  ...
  for (length = 0, blob = info->blobs, end = blob + info->s->base.blobs;
       blob != end; blob++) {
    blob->length =
        _mi_calc_blob_length(blob->pack_length, record + blob->offset);
    length += blob->length;
  }
  return length;
}
...
int _mi_cmp_dynamic_unique(MI_INFO *info, MI_UNIQUEDEF *def,
  ...
  if (!(old_record = (uchar *)my_malloc(mi_key_memory_record_buffer,
                                        info->s->base.reclength, MYF(0))))
    DBUG_RETURN(1);
...
int _mi_cmp_dynamic_record(MI_INFO *info, const uchar *record) {
  ...
  if (info->opt_flag & WRITE_CACHE_USED) {
    info->update &= ~(HA_STATE_WRITE_AT_END | HA_STATE_EXTEND_BLOCK);
    if (flush_io_cache(&info->rec_cache)) DBUG_RETURN(-1);
  }
  info->rec_cache.seek_not_done = 1;
  ...
  if ((info->opt_flag & READ_CHECK_USED)) { /* If check isn't disabled  */
    if (info->s->base.blobs) {
      if (!(buffer =
                (uchar *)my_malloc(mi_key_memory_record_buffer,
  ...
  set_my_errno(0);
err:
  if (buffer != info->rec_buff) my_free((uchar *)buffer);
...
int _mi_read_rnd_dynamic_record(MI_INFO *info, uchar *buf, my_off_t filepos,
  ...
  DBUG_EXECUTE_IF("catch_file_offset_deviation", {
    if (filepos) DBUG_RETURN(HA_ERR_RECORD_DELETED);
  });
  ...
  do {
    ...
    if (block_of_record == 0) /* First block */
    {
      ...
      if (share->base.blobs) {
        if (!(to =
                  mi_alloc_rec_buff(info, block_info.rec_len, &info->rec_buff)))
          goto err;
      } else
        to = info->rec_buff;
    ...
    /* copy information that is already read */
    {
      ...
      if (tmp_length > block_info.data_len) tmp_length = block_info.data_len;
      if (tmp_length) {
    ...
    if (block_info.data_len) {
      if (info->opt_flag & READ_CACHE_USED) {
        ...
        if (mysql_file_read(info->dfile, (uchar *)to, block_info.data_len,
                            MYF(MY_NABP))) {
          if (my_errno() == -1)
            set_my_errno(HA_ERR_WRONG_IN_RECORD); /* Unexpected end of file */
          goto err;
        }
      }
    }
...
uint _mi_get_block_info(MI_BLOCK_INFO *info, File file, my_off_t filepos) {
  ...
  switch (info->header[0]) {
    ...
    case 1:
      info->rec_len = info->data_len = info->block_len =
          mi_uint2korr(header + 1);
      info->filepos = filepos + 3;
      return return_val | BLOCK_FIRST | BLOCK_LAST;
    case 2:
      info->rec_len = info->data_len = info->block_len =
          mi_uint3korr(header + 1);
